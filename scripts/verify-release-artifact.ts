...
function packArtifact() {
  ...
  for (const target of [
    ...
    if (!packagedFiles.has(target))
      fail(`Advertised artifact is missing: ${target}`);
  }
...
function verifyFreshInstall(tarballPath: string) {
  ...
  try {
    ...
    const smokeScript = [
      ...
      "import server from 'oh-my-opencode-slim/server';",
      "if (typeof server.server !== 'function') throw new Error('server export does not load');",
    ...
    run(
      ...
      [
        '--eval',
        "import tui from 'oh-my-opencode-slim/tui'; if (typeof tui.tui !== 'function') throw new Error('TUI export does not load'); process.exit(0);",
      ],
      { cwd: installDir },
    );
  } finally {
