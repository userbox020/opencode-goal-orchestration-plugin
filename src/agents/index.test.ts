...
describe('goal primary agent', () => {
  test('allows execution-owner controls while preserving explicit denials', () => {
    const controls = [
      'task_cancel',
      'task_message',
      'task_revive',
    ...
    const defaults = getAgentConfigs(runtimeFor()).goal.permission as Record<
      string,
      unknown
    >;
    for (const tool of controls) expect(defaults[tool]).toBe('allow');
    const configured = getAgentConfigs(
      runtimeFor({
        agents: {
          goal: { permission: { task_cancel: 'deny', wait_for_user: 'deny' } },
        },
      }),
    ).goal.permission as Record<string, unknown>;
    expect(configured.task_cancel).toBe('deny');
    expect(configured.wait_for_user).toBe('deny');
  });

  test('is a visible primary agent with grounded lifecycle guidance', () => {
    const configs = getAgentConfigs(runtimeFor());

    expect(configs.goal.mode).toBe('primary');
    expect(configs.goal.hidden).toBeUndefined();
    expect(configs.goal.description).toBe(
      'Execute the active /goal, coordinate bounded work, and verify its required criteria.',
    );
    expect(configs.goal.prompt).toContain(
      'first eligible ordinary user message while Goal is selected automatically creates',
    );
    expect(configs.goal.prompt).toContain(
      '/goal <objective> for explicit Goal creation or replacement after a terminal Goal',
    );
    expect(configs.goal.prompt).toContain('/goal status');
    expect(configs.goal.prompt).toContain('no Desktop card integration');
  });

  test('uses the existing override and model configuration path', () => {
    const configs = getAgentConfigs(
      runtimeFor({
        agents: {
          goal: {
            model: 'test/goal-model',
            description: 'Focused goal management',
          },
        },
      }),
    );
    ...
    expect(configs.goal.mode).toBe('primary');
    expect(configs.goal.model).toBe('test/goal-model');
    expect(configs.goal.description).toBe('Focused goal management');
  });

  test('treats a goal configuration as the canonical primary agent', () => {
    const configs = getAgentConfigs(
      runtimeFor({
        agents: {
          goal: { model: 'test/goal-model' },
        },
      }),
    );

    expect(ALL_AGENT_NAMES).toContain('goal');
    expect(DEFAULT_MODELS.goal).toBeUndefined();
    expect(configs.goal.mode).toBe('primary');
    expect(configs.goal.model).toBe('test/goal-model');
    expect(Object.keys(configs).filter((name) => name === 'goal')).toHaveLength(
      1,
    );
  });

  test('rejects ACP agents that collide with Goal', () => {
    expect(() =>
      createAgents(
        runtimeFor({
          acpAgents: {
    ...
    ).toThrow("ACP agent 'goal' conflicts with a built-in agent name or alias");
  });

  test('is omitted when the V2 integration disables Goal', () => {
    const configs = getAgentConfigs(runtimeFor(), { goalEnabled: false });

    expect(configs.goal).toBeUndefined();
  });
});
...
describe('getAgentConfigs', () => {
  test('returns config record keyed by agent name', () => {
    ...
    expect(configs.goal).toBeDefined();
  ...
  test('includes description in SDK config', () => {
    ...
    expect(configs.goal.description).toBeDefined();
  });
