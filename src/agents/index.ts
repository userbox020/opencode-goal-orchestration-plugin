...
import { createGoalAgent } from './goal';
...
function applyDefaultPermissions(
  ...
  const taskControlPermissions = Object.fromEntries(
    TASK_CONTROL_TOOL_NAMES.map((toolName) => [
      toolName,
      existing[toolName] ??
        (agent.name === 'orchestrator' || agent.name === 'goal'
          ? 'allow'
          : 'deny'),
    ]),
  );
...
export function createAgents(
  ...
  options?: { projectDirectory?: string; goalEnabled?: boolean },
): AgentDefinition[] {
  ...
  // Goal is a user-selectable primary agent. It follows the normal prompt,
  // override, model, and permission path while remaining unavailable in V2.
  const goalOverride = getOverrideFromAgents(mergedAgents, 'goal');
  const goalPrompts = loadAgentPrompt('goal', {
    preset: runtime.preset,
    projectDirectory: options?.projectDirectory,
  });
  const goal = createGoalAgent(primaryModel, undefined, undefined);
  goal.config.prompt = resolvePrompt(
    'goal',
    goalOverride?.prompt,
    goalPrompts.prompt,
    goal.config.prompt ?? '',
    goalPrompts.appendPrompt,
  );
  if (goalOverride) {
    applyOverrides(goal, goalOverride);
  }
  applyDefaultPermissions(goal, goalOverride?.skills, runtime.disabledSkills);
  ...
  if (
    options?.goalEnabled !== false &&
    !disabled.has('goal') &&
    goal.displayName
  ) {
    displayNameMap.set('goal', goal.displayName);
  }
  ...
  if (options?.goalEnabled !== false && !disabled.has('goal')) {
    primaryAgents.push(goal);
  }
...
export function getAgentConfigs(
  ...
  const applyClassification = (
    ...
    if (name === 'council') {
      ...
      sdkConfig.mode = 'subagent';
    } else if (name === 'orchestrator' || name === 'goal') {
