...
function reconcileExecutionBatch(
  ...
  for (const execution of executions) {
    ...
    const reconciled = state.backgroundJobBoard.markReconciled(execution.taskID);
    if (reconciled) state.onTerminalReconciled?.(execution);
  }
}
...
function injectLatestBoard(state: InjectionState, messages: unknown[]): void {
  ...
  if (
    trigger.info.agent &&
    trigger.info.agent !== 'orchestrator' &&
    !state.managedAgentNames?.has(trigger.info.agent)
  ) {
    return;
  }
  ...
  const triggerText = textPart.text;
  const internalInitiator = isInternalInitiatorPart(textPart);
  const allowInternalContext =
    internalInitiator &&
    [...(state.additionalContextInternalTexts ?? [])].some((text) =>
      triggerText.startsWith(text),
    );
  if (internalInitiator && !allowInternalContext) return;
  if (internalInitiator) {
    const additionalContext = state.additionalTrailingContext?.(sessionID);
    if (additionalContext) {
      appendTrailingVolatileMessage(
        messages,
        { ...trigger.info, id: `${trigger.info.id ?? 'goal'}-goal-context` },
        { text: additionalContext, metadataKey: state.metadataKey },
      );
    }
    return;
  }
  ...
  const reminder = [
    boardMeta?.text,
    state.additionalTrailingContext?.(sessionID),
  ]
...
function injectCheckpointBoard(
  ...
  const canSurface =
    ...
    (!triggeringMessage.info.agent ||
      ...
      state.managedAgentNames?.has(triggeringMessage.info.agent)) &&
    textPart !== undefined &&
    !isInternalInitiatorPart(textPart);
  const canSurfaceInternalContext =
    triggeringMessage !== undefined &&
    (!triggeringMessage.info.agent ||
      triggeringMessage.info.agent === 'orchestrator' ||
      state.managedAgentNames?.has(triggeringMessage.info.agent)) &&
    triggeringText !== undefined &&
    textPart !== undefined &&
    isInternalInitiatorPart(textPart) &&
    [...(state.additionalContextInternalTexts ?? [])].some((text) =>
      triggeringText.startsWith(text),
    );
  ...
  const additionalContext =
    (canSurface || canSurfaceInternalContext) &&
    state.additionalTrailingContext?.(sessionID);
  if (additionalContext) {
    const contextAnchor = triggeringMessage ?? tailMessage;
    appendTrailingVolatileMessage(
      messages,
      {
        ...contextAnchor.info,
        id: `${contextAnchor.info.id ?? 'goal'}-goal-context`,
      },
      { text: additionalContext, metadataKey: state.metadataKey },
    );
  }
