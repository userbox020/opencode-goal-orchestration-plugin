...
type HookOptions = {
  ...
  registerSessionAsManaged?: (sessionID: string, agent: string) => void;
  additionalTrailingContext?: (sessionID: string) => string | undefined;
  additionalContextInternalTexts?: ReadonlySet<string>;
  ...
  onTaskRegistered?: (record: BackgroundJobRecord) => void | Promise<void>;
  onExternalUserMessage?: (message: {
    sessionID: string;
    messageID: string;
    parts: unknown[];
  }) => void | Promise<void>;
...
function createHook(options?: HookOptions) {
  const hook = createTaskSessionManagerHook(
    ...
    } as never,
    {
      ...
      managedAgentNames: options?.managedAgentNames,
      registerSessionAsManaged: options?.registerSessionAsManaged,
      additionalTrailingContext: options?.additionalTrailingContext,
      additionalContextInternalTexts: options?.additionalContextInternalTexts,
      isFallbackInProgress: options?.isFallbackInProgress,
      willAttemptFallback: options?.willAttemptFallback,
      coordinator: options?.coordinator,
      onTaskRegistered: options?.onTaskRegistered,
      onExternalUserMessage: options?.onExternalUserMessage,
...
describe('task-session-manager hook', () => {
  ...
  test('does not register a historical launch that has a completed terminal part', async () => {
    const board = new BackgroundJobBoard();
    const onTaskRegistered = mock(async () => {});
    const { hook } = createHook({
      backgroundJobBoard: board,
      onTaskRegistered,
      runtimeStatusReconcileDelayMs: 60_000,
    ...
    const messages = {
      messages: [
        {
          info: { role: 'assistant', sessionID: 'parent-1' },
          parts: [historicalRunningTaskPart('completed-historical-child')],
        },
        {
          info: { role: 'user', agent: 'orchestrator', sessionID: 'parent-1' },
          parts: [
            {
              ...
              synthetic: true,
              text: [
                '<task id="completed-historical-child" state="completed">',
                '<summary>Background task completed: recovered</summary>',
                '<task_result>',
                'historical result',
                '</task_result>',
    ...
    expect(board.get('completed-historical-child')).toMatchObject({
      state: 'completed',
    });
    expect(onTaskRegistered).not.toHaveBeenCalled();
  });
  ...
  test.each(['busy', 'retry'] as const)(
    'registers a rehydrated child exactly once after live %s confirmation',
    async (status) => {
      const board = new BackgroundJobBoard();
      const onTaskRegistered = mock(async () => {});
      const { hook } = createHook({
        backgroundJobBoard: board,
        onTaskRegistered,
        sessionStatus: { 'historical-child': { type: status } },
        runtimeStatusReconcileDelayMs: 60_000,
      });
      const messages = {
        messages: [
      ...
      expect(onTaskRegistered).toHaveBeenCalledTimes(1);
      expect(onTaskRegistered).toHaveBeenCalledWith(
        expect.objectContaining({
          taskID: 'historical-child',
          generation: board.get('historical-child')?.generation,
          state: 'running',
        }),
      );
    },
  ...
  test.each([
    ['idle', { 'historical-child': { type: 'idle' } }],
    ['absent', {}],
  ])('does not register a rehydrated child when live status is %s', async (_label, sessionStatus) => {
    const board = new BackgroundJobBoard();
    const onTaskRegistered = mock(async () => {});
    const { hook } = createHook({
      backgroundJobBoard: board,
  ...
  test('registers a direct live background launch immediately', async () => {
    const onTaskRegistered = mock(async () => {});
    const { hook } = createHook({ onTaskRegistered });

    await hook['tool.execute.before'](
      { tool: 'task', sessionID: 'parent-1', callID: 'live-call' },
      {
        args: {
          subagent_type: 'explorer',
          background: true,
          description: 'inspect live launch',
        },
      },
    );
    await hook['tool.execute.after'](
      { tool: 'task', sessionID: 'parent-1', callID: 'live-call' },
      { output: taskLaunchOutput('live-child') },
    );

    expect(onTaskRegistered).toHaveBeenCalledTimes(1);
    expect(onTaskRegistered).toHaveBeenCalledWith(
      expect.objectContaining({ taskID: 'live-child', state: 'running' }),
    );
  });
  ...
  test('recovers a Goal-owned session in messages.transform', async () => {
    const agentMap = new Map<string, string>([['goal-1', 'explorer']]);
    const board = new BackgroundJobBoard();
    board.registerLaunch({
      taskID: 'child-goal-1',
      parentSessionID: 'goal-1',
      agent: 'explorer',
      description: 'Goal transform recovery test',
    });
    const { hook } = createHook({
      backgroundJobBoard: board,
      shouldManageSession: (id) =>
        agentMap.get(id) === 'orchestrator' || agentMap.get(id) === 'goal',
      managedAgentNames: new Set(['orchestrator', 'goal']),
    ...
    const messages = {
      messages: [
        {
          info: { role: 'user', agent: 'goal', sessionID: 'goal-1' },
          parts: [{ type: 'text', text: 'continue the Goal' }],
        },
      ],
    };
    ...
    expect(agentMap.get('goal-1')).toBe('goal');
    expect(boardText(messages)).toContain('### Background Job Board');
    expect(boardText(messages)).toContain('child-goal-1');
  });

  for (const strategy of ['latest', 'checkpoint-compatible'] as const) {
    test(`injects Goal context on authorized ${strategy} internal wakes only`, async () => {
      const { hook } = createHook({
        strategy,
        managedAgentNames: new Set(['orchestrator', 'goal']),
        additionalTrailingContext: () => 'Goal: ship the fix',
        additionalContextInternalTexts: new Set(['Authorized Goal wake.']),
      });
      const authorized = {
        messages: [
          {
            info: { role: 'user', agent: 'goal', sessionID: 'goal-1' },
            parts: [createInternalAgentTextPart('Authorized Goal wake.')],
          },
        ],
      };
      const unrelated = {
        messages: [
          {
            info: { role: 'user', agent: 'goal', sessionID: 'goal-1' },
            parts: [createInternalAgentTextPart('Unrelated internal turn.')],
          },
        ],
      };

      await transformMessages(hook, authorized as never);
      await transformMessages(hook, unrelated as never);

      expect(boardText(authorized) ?? '').toContain('Goal: ship the fix');
      expect(boardText(unrelated) ?? '').not.toContain('Goal: ship the fix');
    });
  }
  ...
  test('calls the external-user callback with the classified message payload', async () => {
    const onExternalUserMessage = mock(async () => {});
    const { hook } = createHook({ onExternalUserMessage });
    const parts = [{ type: 'text', text: 'Create a Goal for this work.' }];

    await hook.observeChatMessage(
      { sessionID: 'parent-1', messageID: 'msg-goal-request' },
      {
        message: {
          id: 'msg-goal-request',
          role: 'user',
    ...
    expect(onExternalUserMessage).toHaveBeenCalledWith({
      sessionID: 'parent-1',
      messageID: 'msg-goal-request',
      parts,
    });
  });

  test('awaits the external-user callback after clearing wait_for_user', async () => {
    let resolveCallback: (() => void) | undefined;
    const callbackPending = new Promise<void>((resolve) => {
      resolveCallback = resolve;
    });
    const onExternalUserMessage = mock(() => callbackPending);
    const { hook } = createHook({ onExternalUserMessage });
    hook.beginUserWait('parent-1');
    ...
    const observed = hook
      .observeChatMessage(
        { sessionID: 'parent-1', messageID: 'msg-await-callback' },
        {
          message: {
            id: 'msg-await-callback',
            role: 'user',
            sessionID: 'parent-1',
          },
          parts: [{ type: 'text', text: 'Continue after review.' }],
        },
      )
      .then(() => {
        completed = true;
    ...
    expect(hook.hasInputWait('parent-1')).toBe(false);
    expect(onExternalUserMessage).toHaveBeenCalledTimes(1);
    await Promise.resolve();
  ...
  test.each([
    [
      'assistant messages',
      { sessionID: 'parent-1', messageID: 'msg-assistant' },
      {
        message: { id: 'msg-assistant', role: 'assistant', sessionID: 'parent-1' },
        parts: [{ type: 'text', text: 'assistant response' }],
      },
    ],
    [
      'unmanaged sessions',
      { sessionID: 'unmanaged-1', messageID: 'msg-unmanaged' },
      {
        message: { id: 'msg-unmanaged', role: 'user', sessionID: 'unmanaged-1' },
        parts: [{ type: 'text', text: 'external request' }],
      },
    ],
    [
      'missing stable message identity',
      { sessionID: 'parent-1' },
      {
    ...
    [
      'synthetic messages',
      { sessionID: 'parent-1', messageID: 'msg-synthetic' },
      {
        message: { id: 'msg-synthetic', role: 'user', sessionID: 'parent-1' },
        parts: [{ type: 'text', synthetic: true, text: 'synthetic continuation' }],
      },
    ],
    [
      'internal wake messages',
      { sessionID: 'parent-1', messageID: 'msg-wake' },
      {
        message: { id: 'msg-wake', role: 'user', sessionID: 'parent-1' },
        parts: [
          { type: 'text', text: 'external request' },
          createInternalAgentTextPart('Authorized Goal wake.'),
        ],
      },
    ],
    [
      'compaction continuation messages',
      { sessionID: 'parent-1', messageID: 'msg-compaction' },
      {
        message: { id: 'msg-compaction', role: 'user', sessionID: 'parent-1' },
        parts: [
          { type: 'text', text: 'external request' },
          {
            type: 'text',
    ...
    [
      'messages without real text, file, or image parts',
      { sessionID: 'parent-1', messageID: 'msg-tool-only' },
      {
        message: { id: 'msg-tool-only', role: 'user', sessionID: 'parent-1' },
        parts: [{ type: 'tool', tool: 'read' }],
      },
    ],
  ])('does not call the external-user callback for %s', async (_reason, input, output) => {
    const onExternalUserMessage = mock(async () => {});
    const { hook } = createHook({
    ...
    await hook.observeChatMessage(input, output);

    expect(onExternalUserMessage).not.toHaveBeenCalled();
  });
