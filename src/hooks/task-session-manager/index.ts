...
async function rehydrateHistoricalRunningTasks(
  messages: unknown[],
  backgroundJobBoard: BackgroundJobStore,
  shouldManageSession: (sessionID: string) => boolean,
  registerSessionAsOrchestrator?: (sessionID: string) => void,
  managedAgentNames: ReadonlySet<string> = new Set(['orchestrator']),
  ...
  for (const message of messages) {
    ...
    if (!message.info.agent || !managedAgentNames.has(message.info.agent)) continue;

    const parentSessionID = message.info.sessionID;
    if (!parentSessionID) continue;
    if (!shouldManageSession(parentSessionID)) {
      if (message.info.agent === 'orchestrator') {
        registerSessionAsOrchestrator?.(parentSessionID);
      }
      registerSessionAsManaged?.(parentSessionID, message.info.agent);
      if (!shouldManageSession(parentSessionID)) continue;
    }
    managedSessionIDs.add(parentSessionID);
  }
  ...
  for (const message of messages) {
    ...
    if (
      !parentSessionID ||
      !managedSessionIDs.has(parentSessionID)
    ) {
      continue;
    }
    ...
    for (const part of message.parts) {
      ...
      // Historical task output only proves that this task once launched. Keep
      // it provisional until the runtime probe confirms this board generation
      // is actively busy or retrying.
      pendingLiveRunning?.set(taskID, record.generation);
      rehydrated += 1;
    }
  }
...
export function createTaskSessionManagerHook(
  ...
  options: {
    ...
    /** Called only after this manager classifies a real external user message. */
    ...
    onTerminalReconciled?: (execution: BackgroundJobExecution) => void;
  },
) {
  ...
  const pendingHistoricalLiveRunning = new Map<string, number>();
  ...
  const runtimeStatusReconciler = createRuntimeStatusReconciler({
    ...
    taskContextTracker,
    pendingLiveRunning: pendingHistoricalLiveRunning,
    onLiveRunning: options.onTaskRegistered,
  });
  ...
  const injectionState: InjectionState = {
    ...
    onTerminalReconciled: options.onTerminalReconciled,
  };
  ...
  return {
    ...
    observeChatMessage: async (input: unknown, output: unknown): Promise<void> => {
      ...
      const role =
        typeof outputMessage?.role === 'string'
          ? outputMessage.role
          : typeof inputMessage?.role === 'string'
            ? inputMessage.role
            : undefined;
      if (
        !sessionID ||
      ...
      if (stableMessageID) {
        await options.onExternalUserMessage?.({
          sessionID,
    ...
    'experimental.chat.messages.transform': async (
      ...
      const rehydratedCount = await rehydrateHistoricalRunningTasks(
        messages,
        backgroundJobBoard,
        options.shouldManageSession,
        options.registerSessionAsOrchestrator,
        options.managedAgentNames,
        options.registerSessionAsManaged,
        rehydrateTombstones,
        pendingHistoricalLiveRunning,
      );
      ...
      for (const [messageIndex, message] of messages.entries()) {
        ...
        if (
          ...
          !options.managedAgentNames?.has(message.info.agent)
        ) {
          continue;
        }
        if (
          ...
          options.registerSessionAsManaged?.(sessionID, message.info.agent);
