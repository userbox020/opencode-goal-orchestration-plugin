...
import { BackgroundJobBoard, type BackgroundJobRecord } from '../../utils';
...
describe('runtime status reconciliation', () => {
  ...
  test.each(['busy', 'retry'] as const)(
    'calls onLiveRunning once for a pending historical generation confirmed %s',
    async (type) => {
      const pendingLiveRunning = new Map([['child-1', 1]]);
      const onLiveRunning = mock(async () => {});
      const { board, reconciler } = createReconciler(
        async () => ({ data: { 'child-1': { type } } }),
        undefined,
        undefined,
        { pendingLiveRunning, onLiveRunning },
      );

      await reconciler.reconcile();
      await reconciler.reconcile();

      expect(onLiveRunning).toHaveBeenCalledTimes(1);
      expect(onLiveRunning).toHaveBeenCalledWith(
      ...
      expect(pendingLiveRunning.size).toBe(0);
      expect(board.get('child-1')).toMatchObject({ state: 'running' });
    },
  );

  test.each([
    ['idle', async () => ({ data: { 'child-1': { type: 'idle' } } })],
    ['absent', async () => ({ data: {} })],
    ['malformed', async () => ({ data: { 'child-1': { type: 'suspended' } } })],
    ['error', async () => {
      throw new Error('server restarting');
    }],
  ])(
    'retains a pending historical task through %s before live confirmation',
    async (_label, status) => {
      const pendingLiveRunning = new Map([['child-1', 1]]);
      const onLiveRunning = mock(async () => {});
      let attempt = 0;
      ...
      expect(onLiveRunning).not.toHaveBeenCalled();
      expect(pendingLiveRunning.get('child-1')).toBe(1);
  ...
  test('drops a stale pending historical generation without registering it', async () => {
    ...
    response.resolve({ data: { 'child-1': { type: 'busy' } } });
