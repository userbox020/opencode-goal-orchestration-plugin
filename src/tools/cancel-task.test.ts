...
function createTool(overrides?: {
  ...
  const tools = createCancelTaskTool({
    ...
    resolveAgentName: (agent) => (agent === 'mission' ? 'goal' : agent),
    verifyAbortMs: 10,
...
describe('task_cancel tool', () => {
  test('recognizes a configured Goal display alias', async () => {
    ...
    await expect(
      taskCancel.execute(
        { task_id: 'ses_1' },
        { ...context, agent: 'mission' },
      ),
    ).resolves.toContain('cancelled');
  });

  test('permits a Goal owner but still rejects unowned tasks', async () => {
    const { board, taskCancel } = createTool();
    board.registerLaunch({
      taskID: 'ses_1',
      parentSessionID: 'parent-1',
    ...
    await expect(
      taskCancel.execute({ task_id: 'ses_1' }, { ...context, agent: 'goal' }),
    ).resolves.toContain('cancelled');
    await expect(
    ...
    ).resolves.toContain('state: unknown');
  });
